...
function App(){
  ...
  const reloadTable=useCallback(async(table)=>{const client=sb();if(!client)return;const m=tableMap[table];if(!m)return;let q=client.from(m.view||table).select("*").order(m.order,{ascending:m.asc});if(m.limit)q=q.limit(m.limit);let{data:d}=await q;
