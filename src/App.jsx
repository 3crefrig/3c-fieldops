...
function ProjectDetail({project,onBack,onUpdate,onDelete,users,userName,userRole,allWOs,onCreateWO,allPOs,allTime,customers}){
  ...
  return(<div><Toast msg={toast}/>
    ...
    <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}><StatCard label="Milestones" value={mDone+"/"+mTot} icon="🎯" color={mTot>0&&mDone===mTot?B.green:B.cyan}/><StatCard label="Parts" value={pDone+"/"+pTot} icon="🔩" color={pTot>0&&pDone===pTot?B.green:B.orange}/><StatCard label="WOs" value={pWOs.length} icon="📋" color={B.cyan}/><StatCard label="Hours" value={pHrs.toFixed(1)+"h"} icon="⏱" color={B.orange}/>{allProjectPOs.filter(p=>p.status==="pending").length>0&&<StatCard label="Pending POs" value={allProjectPOs.filter(p=>p.status==="pending").length} icon="📄" color={B.orange}/>}{isMgr&&project.budget>0&&<StatCard label="Budget Left" value={"$"+bLeft.toLocaleString()} icon="💰" color={bLeft<0?B.red:B.green}/>}</div>
    ...
    {tab==="pos"&&<div>
      ...
      {allProjectPOs.filter(p=>!selChamber||p.chamber_id===selChamber).map(po=>{const chName=po.chamber_id?chambers.find(c=>c.id===po.chamber_id)?.name:null;return<Card key={po.id} style={{padding:"12px 16px",marginBottom:6,borderLeft:"3px solid "+(PSC[po.status]||B.border)}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontFamily:M,fontSize:15,fontWeight:800,color:B.text}}>{"$"+(parseFloat(po.amount)||0).toFixed(2)}</span>
