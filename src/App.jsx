...
function App(){
  ...
  const loadData=useCallback(async()=>{try{const client=sb();if(!client)return;
    const[wos,pos,time,photos,users,schedule,templates,notifs,customers,emailTemplates,projects,woDrafts,invoices,feedbackData,equipmentData,agreementsData,agreementTiersData,lineItemsData,rfqsData,rfqItemsData,rfqSpecsData]=await Promise.all([
      ...
      client.from("rfq_items_public").select("*").order("line_no",{ascending:true}),
      client.from("rfq_specs").select("*").order("created_at",{ascending:true}),
    ]);
    [wos,pos,time,photos,users,schedule,templates,notifs,customers,emailTemplates,projects,woDrafts,invoices,feedbackData,equipmentData,agreementsData,agreementTiersData,lineItemsData,rfqsData,rfqItemsData,rfqSpecsData].forEach((r,i)=>{if(r.error)console.warn("loadData query "+i+" failed:",r.error.message);});
    const freshData={wos:wos.data||[],pos:pos.data||[],time:time.data||[],photos:photos.data||[],users:users.data||[],schedule:schedule.data||[],templates:templates.data||[],notifs:notifs.data||[],customers:customers.data||[],emailTemplates:emailTemplates.data||[],projects:projects.data||[],woDrafts:woDrafts.data||[],invoices:invoices.data||[],feedback:feedbackData.data||[],equipment:equipmentData.data||[],agreements:agreementsData.data||[],agreementTiers:agreementTiersData.data||[],lineItems:lineItemsData.data||[],rfqs:rfqsData.data||[],rfqItems:rfqItemsData.data||[],rfqSpecs:rfqSpecsData.data||[]};
  ...
  // Silently refresh this device's push subscription once we know who's logged in
  // (only if they've already granted notification permission — no prompt here).
  useEffect(()=>{if(appUser?.id&&typeof Notification!=="undefined"&&Notification.permission==="granted"){registerPush(appUser.id,{promptIfNeeded:false});}},[appUser?.id]);
  ...
  useEffect(()=>{if(!authUser)return;const client=sb();
    ...
    checkWarranties();
    // Check agreement renewals (expiring within 30 days)
    const checkAgreementRenewals=async()=>{const today=new Date().toISOString().slice(0,10);const thirtyDays=new Date(Date.now()+30*86400000).toISOString().slice(0,10);const{data:expiring}=await client.from("service_agreements").select("id,agreement_num,customer_name,end_date").eq("status","active").eq("renewal_reminder_sent",false).gte("end_date",today).lte("end_date",thirtyDays);if(expiring&&expiring.length>0){for(const a of expiring){await client.from("notifications").insert({type:"agreement_expiring",title:"Agreement Expiring",message:a.agreement_num+" for "+a.customer_name+" expires "+a.end_date,for_role:"manager"});await client.from("service_agreements").update({renewal_reminder_sent:true}).eq("id",a.id);}}};
    ...
    checkOverdueWOs();
    // Overdue by DUE DATE — any not-yet-completed WO whose due_date (a real date) has passed.
    const checkDueDateOverdue=async()=>{const today=new Date().toISOString().slice(0,10);const{data:wos}=await client.from("work_orders").select("id,wo_id,title,assignee,due_date,status").neq("status","completed");const overdue=(wos||[]).filter(w=>/^\d{4}-\d{2}-\d{2}$/.test(w.due_date||"")&&w.due_date<today);for(const w of overdue){const{data:existing}=await client.from("notifications").select("id").eq("type","wo_past_due").ilike("message","%"+w.wo_id+"%").gte("created_at",today);if(!existing||existing.length===0){await client.from("notifications").insert({type:"wo_past_due",title:"Work Order Overdue",message:(w.wo_id||"")+" — "+(w.title||"")+" was due "+w.due_date,for_role:null});pushSend({userNames:w.assignee&&w.assignee!=="Unassigned"?[w.assignee]:[],roles:["manager"],title:"Overdue Work Order",body:(w.wo_id||"")+" was due "+w.due_date,url:"/#tab=orders",emailFallback:true});}}};
    checkDueDateOverdue();
    // Invoice backlog — completed, un-invoiced, non-project WOs for billable customers
    // (excludes School of Medicine / Duke Facilities Maintenance and auto-invoice customers).
    const checkInvoiceBacklog=async()=>{const today=new Date().toISOString().slice(0,10);const cut30=new Date(Date.now()-30*86400000).toISOString().slice(0,10);const{data:done}=await client.from("work_orders").select("id,wo_id,customer,status,invoiced,project_id,date_completed").eq("status","completed").eq("invoiced",false).is("project_id",null);if(!done||done.length===0)return;const{data:custs}=await client.from("customers").select("name,auto_invoice");const autoNames=new Set((custs||[]).filter(c=>c.auto_invoice).map(c=>c.name));const recent=w=>/^\d{4}-\d{2}-\d{2}$/.test(w.date_completed||"")&&w.date_completed>=cut30;const ready=done.filter(w=>recent(w)&&(w.customer||"").trim()!==""&&!isInvoiceExcludedCustomer(w.customer)&&!autoNames.has(w.customer));for(const w of ready){const{data:existing}=await client.from("notifications").select("id").eq("type","wo_needs_invoice").ilike("message","%"+w.wo_id+"%").gte("created_at",today);if(!existing||existing.length===0)await client.from("notifications").insert({type:"wo_needs_invoice",title:"Ready to Invoice",message:(w.wo_id||"")+" — "+(w.customer||"customer")+" is complete and needs invoicing",for_role:"manager"});}};
    checkInvoiceBacklog();
    // Unassigned WO sitting >24h — dispatch gap (managers + push).
    const checkUnassignedWOs=async()=>{const today=new Date().toISOString().slice(0,10);const cutoff=new Date(Date.now()-24*3600000).toISOString();const{data:wos}=await client.from("work_orders").select("id,wo_id,title,assignee,status,created_at").neq("status","completed").lte("created_at",cutoff);const un=(wos||[]).filter(w=>!w.assignee||w.assignee==="Unassigned"||String(w.assignee).trim()==="");for(const w of un){const{data:existing}=await client.from("notifications").select("id").eq("type","wo_unassigned").ilike("message","%"+w.wo_id+"%").gte("created_at",today);if(!existing||existing.length===0){await client.from("notifications").insert({type:"wo_unassigned",title:"Unassigned Work Order",message:(w.wo_id||"")+" — "+(w.title||"")+" has been unassigned over 24h",for_role:"manager"});pushSend({roles:["manager","admin"],title:"Unassigned Work Order",body:(w.wo_id||"")+" needs a tech (24h+)",url:"/#tab=orders"});}}};
    checkUnassignedWOs();
    // PO stuck awaiting approval >24h — techs waiting on parts (managers + push).
    const checkStalePOApprovals=async()=>{const today=new Date().toISOString().slice(0,10);const cutoff=new Date(Date.now()-24*3600000).toISOString();const{data:pos}=await client.from("purchase_orders").select("id,po_id,amount,requested_by,status,created_at").eq("status","pending").lte("created_at",cutoff);for(const p of (pos||[])){const{data:existing}=await client.from("notifications").select("id").eq("type","po_approval_stale").ilike("message","%"+p.po_id+"%").gte("created_at",today);if(!existing||existing.length===0){await client.from("notifications").insert({type:"po_approval_stale",title:"PO Awaiting Approval",message:(p.po_id||"")+" — $"+(p.amount||0)+" from "+(p.requested_by||"")+" pending over 24h",for_role:"manager"});pushSend({roles:["manager","admin"],title:"PO Awaiting Approval",body:(p.po_id||"")+" — $"+(p.amount||0)+" pending 24h+",url:"/#tab=pos"});}}};
    checkStalePOApprovals();
    // Completed WO missing hours or signature before it can be invoiced (managers, bell only).
    const checkCompletionGaps=async()=>{const today=new Date().toISOString().slice(0,10);const cut30g=new Date(Date.now()-30*86400000).toISOString().slice(0,10);const{data:done}=await client.from("work_orders").select("id,wo_id,title,customer,signature,invoiced,project_id,date_completed").eq("status","completed").eq("invoiced",false).is("project_id",null);if(!done||done.length===0)return;const rel=done.filter(w=>/^\d{4}-\d{2}-\d{2}$/.test(w.date_completed||"")&&w.date_completed>=cut30g&&(w.customer||"").trim()!==""&&!isInvoiceExcludedCustomer(w.customer));if(rel.length===0)return;const ids=rel.map(w=>w.id);const{data:times}=await client.from("time_entries").select("wo_id,hours").in("wo_id",ids);const hrs={};(times||[]).forEach(t=>{hrs[t.wo_id]=(hrs[t.wo_id]||0)+parseFloat(t.hours||0);});for(const w of rel){const noHours=!(hrs[w.id]>0);const noSig=!w.signature;if(!noHours&&!noSig)continue;const gaps=[];if(noHours)gaps.push("no hours logged");if(noSig)gaps.push("no signature");const{data:existing}=await client.from("notifications").select("id").eq("type","wo_completion_gap").ilike("message","%"+w.wo_id+"%").gte("created_at",today);if(!existing||existing.length===0){await client.from("notifications").insert({type:"wo_completion_gap",title:"Completed WO Needs Attention",message:(w.wo_id||"")+" — "+(w.title||"")+": "+gaps.join(" + ")+" before invoicing",for_role:"manager"});}}};
    checkCompletionGaps();
    // Draft invoice sitting unsent >3 days — completed work not yet billed (admins, bell only).
    const checkStaleInvoiceDrafts=async()=>{const today=new Date().toISOString().slice(0,10);const cutoff=new Date(Date.now()-3*86400000).toISOString();const{data:drafts}=await client.from("invoices").select("id,invoice_num,customer,amount,status,created_at").eq("status","draft").lte("created_at",cutoff);for(const inv of (drafts||[])){const key=inv.invoice_num||inv.id;const{data:existing}=await client.from("notifications").select("id").eq("type","invoice_draft_stale").ilike("message","%"+key+"%").gte("created_at",today);if(!existing||existing.length===0){await client.from("notifications").insert({type:"invoice_draft_stale",title:"Draft Invoice Unsent",message:(inv.invoice_num||"Invoice")+" for "+(inv.customer||"")+" — $"+(inv.amount||0)+" still in draft 3+ days",for_role:"admin"});}}};
    checkStaleInvoiceDrafts();
  ...
  // Optimistically reflect a queued offline mutation in local state so the UI updates immediately.
  const applyOptimistic=(table,action,row)=>{const m=tableMap[table];if(!m)return;setData(prev=>{if(!prev)return prev;const arr=prev[m.key]||[];let next;if(action==="insert")next=[{...row,id:row.id||("offline-"+Date.now()),_pending:true},...arr];else if(action==="update")next=arr.map(x=>x.id===row.id?{...x,...row,_pending:true}:x);else next=arr.filter(x=>x.id!==row.id);return{...prev,[m.key]:next};});};
  ...
  const withTableSync=(table,fn)=>async(...args)=>{setSyncing(true);try{await fn(...args);if(!isOffline()){try{await reloadTable(table);}catch(e){}}}catch(e){if(isOffline()||isNetErr(e)){/* offline: queueable actions already queued + returned; non-queueable ones simply can't run */if(e?.__queued)return;alert("You're offline — this change needs a connection and was not saved.");return;}console.error(table+" operation failed:",e);if(e?.message)alert("Operation failed: "+e.message);throw e;}finally{setSyncing(false);}};
  ...
  const actions={
    ...
    createWO:withTableSync("work_orders",async(wo)=>{if(wo.title)wo.title=autoCorrect(wo.title);if(wo.notes)wo.notes=autoCorrect(wo.notes);if(wo.location)wo.location=autoCorrect(wo.location);const{data:ex}=await sb().from("work_orders").select("wo_id").order("wo_id",{ascending:false}).limit(1);const ln=ex&&ex[0]?parseInt(ex[0].wo_id.replace("WO-",""))||1000:1000;const newId="WO-"+(ln+1);const{data:inserted,error:woErr}=await sb().from("work_orders").insert({...wo,wo_id:newId,status:"pending",hours_total:0}).select("id").single();if(woErr){alert("Failed to create work order.");throw woErr;}notify("wo_created","New Work Order",newId+": "+wo.title);if(wo.assignee&&wo.assignee!=="Unassigned")pushSend({userNames:[wo.assignee],title:"New Work Order",body:newId+": "+wo.title,url:"/#tab=orders",emailFallback:true});if(inserted){sb().from("wo_activity").insert({wo_id:inserted.id,action:"created",details:"Work order created",actor:appUser?.name||"System"});evaluateTriggers("wo_created",{...wo,wo_id:newId,id:inserted.id});}}),
    updateWO:withTableSync("work_orders",async(wo)=>{if(wo.title)wo.title=autoCorrect(wo.title);if(wo.notes)wo.notes=autoCorrect(wo.notes);if(wo.work_performed)wo.work_performed=autoCorrect(wo.work_performed);if(wo.field_notes)wo.field_notes=autoCorrect(wo.field_notes);const{id,...rest}=wo;const old=data.wos.find(w=>w.id===id);if(isOffline()){await queueOffline("work_orders","update",{id,...rest});return;}const{error}=await sb().from("work_orders").update(rest).eq("id",id);if(error){console.error("updateWO error:",error);alert("Failed to update work order.");throw error;}const changes=[];if(old){if(rest.status&&rest.status!==old.status)changes.push("Status → "+rest.status);if(rest.assignee&&rest.assignee!==old.assignee)changes.push("Assigned → "+rest.assignee);if(rest.priority&&rest.priority!==old.priority)changes.push("Priority → "+rest.priority);if(rest.tms_entered!==undefined&&rest.tms_entered!==old.tms_entered)changes.push(rest.tms_entered?"TMS marked complete":"TMS unmarked");}if(changes.length>0)sb().from("wo_activity").insert({wo_id:id,action:"updated",details:changes.join(", "),actor:appUser?.name||"System"});if(rest.assignee&&rest.assignee!==old?.assignee&&rest.assignee!=="Unassigned"){notify("wo_assigned","WO Assigned",(old?.wo_id||"")+" assigned to "+rest.assignee,"technician");pushSend({userNames:[rest.assignee],title:"Work Order Assigned",body:(old?.wo_id||"")+" — "+(old?.title||rest.title||""),url:"/#tab=orders",emailFallback:true});}if(rest.status==="completed"&&old?.status!=="completed"){notify("wo_completed","WO Completed",(old?.wo_id||"")+" completed","admin");tryAutoInvoice({...old,...rest});const woC={...old,...rest};const custC=data.customers.find(c=>c.name===woC.customer);if(woReadyToInvoice(woC)&&!(custC&&custC.auto_invoice)){notify("wo_needs_invoice","Ready to Invoice",(woC.wo_id||"")+" — "+(woC.customer||"customer")+" is complete and needs invoicing","manager");pushSend({roles:["manager","admin"],title:"Ready to Invoice",body:(woC.wo_id||"")+" · "+(woC.customer||""),url:"/#tab=billing"});}evaluateTriggers("wo_completed",{...old,...rest});}if(rest.status&&rest.status!==old?.status)evaluateTriggers("wo_status_changed",{...old,...rest,status:rest.status});}),
    deleteWO:withTableSync("work_orders",async(id)=>{const wo=data.wos.find(w=>w.id===id);if(wo?.invoiced){alert("This WO has been invoiced and cannot be deleted.");return;}const{error}=await sb().from("work_orders").delete().eq("id",id);if(error){console.error("deleteWO error:",error);alert("Failed to delete work order.");throw error;}}),
    createPO:withTableSync("purchase_orders",async(po)=>{if(po.amount&&parseFloat(po.amount)<0){alert("PO amount cannot be negative.");return;}if(po.description)po.description=autoCorrect(po.description);if(po.notes)po.notes=autoCorrect(po.notes);const wo=po.wo_id?data.wos.find(w=>w.id===po.wo_id):null;const projId=wo?.project_id||null;const{data:all}=await sb().from("purchase_orders").select("po_id");const id=projId?genProjectPO(all||[]):genPO(all||[]);const{data:settingsRow}=await sb().from("app_settings").select("value").eq("key","app_settings").single();const threshold=parseFloat(settingsRow?.value?.po_auto_approve_threshold)||0;const autoApprove=threshold>0&&parseFloat(po.amount||0)<threshold;const payload={...po,po_id:id,requested_by:appUser.name,status:autoApprove?"approved":"pending"};if(autoApprove){payload.approved_by="auto (under $"+threshold+")";payload.approved_at=new Date().toISOString();}if(projId)payload.project_id=projId;const{error}=await sb().from("purchase_orders").insert(payload);if(error){console.error("createPO error:",error);alert("Failed to create PO. Please try again.");throw error;}if(autoApprove){notify("po_approved","PO Auto-Approved",id+" — $"+po.amount+" (under threshold)","technician");}else{notify("po_requested","PO Requested",id+" — $"+po.amount+" by "+appUser.name,"manager");pushSend({roles:["manager","admin"],title:"PO Approval Needed",body:id+" — $"+(po.amount||0)+" · "+appUser.name,url:"/#tab=pos"});evaluateTriggers("po_requested",{...po,po_id:id});}}),
    updatePO:withTableSync("purchase_orders",async(po)=>{const{id,...rest}=po;const old=data.pos.find(p=>p.id===id);const{error}=await sb().from("purchase_orders").update(rest).eq("id",id);if(error){console.error("updatePO error:",error);alert("Failed to update PO. Please try again.");throw error;}if(rest.status&&rest.status!==old?.status){if(rest.status==="approved"){await notify("po_approved","PO Approved",(old?.po_id||"")+" approved — $"+(rest.amount||old?.amount||0),"technician");if(old?.requested_by)pushSend({userNames:[old.requested_by],title:"PO Approved",body:(old?.po_id||"")+" — $"+(rest.amount||old?.amount||0),url:"/#tab=pos",emailFallback:true});evaluateTriggers("po_approved",{...old,...rest});}if(rest.status==="rejected"){await notify("po_rejected","PO Rejected",(old?.po_id||"")+" rejected","technician");if(old?.requested_by)pushSend({userNames:[old.requested_by],title:"PO Rejected",body:(old?.po_id||"")+" was rejected",url:"/#tab=pos",emailFallback:true});evaluateTriggers("po_rejected",{...old,...rest});}}}),
    deletePO:withTableSync("purchase_orders",async(po)=>{const{error}=await sb().from("purchase_orders").delete().eq("id",po.id);if(error){console.error("deletePO error:",error);alert("Failed to delete PO. Please try again.");throw error;}}),
    createInvoice:withTableSync("invoices",async(inv)=>{const{error}=await sb().from("invoices").insert(inv);if(error){alert("Failed to create invoice.");throw error;}}),
    ...
    deleteCustomer:withTableSync("customers",async(id)=>{const{error}=await sb().from("customers").delete().eq("id",id);if(error){alert("Failed to delete customer.");throw error;}}),
    addEmailTemplate:withTableSync("email_templates",async(t)=>{const{error}=await sb().from("email_templates").insert(t);if(error){alert("Failed to add email template.");throw error;}}),
    updateEmailTemplate:withTableSync("email_templates",async(t)=>{const{id,...rest}=t;const{error}=await sb().from("email_templates").update(rest).eq("id",id);if(error){alert("Failed to update email template.");throw error;}}),
    deleteEmailTemplate:withTableSync("email_templates",async(id)=>{const{error}=await sb().from("email_templates").delete().eq("id",id);if(error){alert("Failed to delete email template.");throw error;}}),
    addProject:withTableSync("projects",async(p)=>{const{error}=await sb().from("projects").insert(p);if(error){alert("Failed to add project.");throw error;}}),
    ...
    approveDraft:async(draft,edits)=>{setSyncing(true);try{
      ...
      notify("wo_created","New Work Order (Email)",newId+": "+wo.title);
      reloadTable("work_orders");reloadTable("wo_drafts");
    }finally{setSyncing(false);}},
    addEquipment:withTableSync("equipment",async(eq)=>{if(eq.notes)eq.notes=autoCorrect(eq.notes);const{error}=await sb().from("equipment").insert(eq);if(error){alert("Failed to add equipment.");throw error;}}),
    updateEquipment:withTableSync("equipment",async(eq)=>{if(eq.notes)eq.notes=autoCorrect(eq.notes);const{id,...rest}=eq;rest.updated_at=new Date().toISOString();const{error}=await sb().from("equipment").update(rest).eq("id",id);if(error){alert("Failed to update equipment.");throw error;}}),
    deleteEquipment:withTableSync("equipment",async(id)=>{const{error}=await sb().from("equipment").delete().eq("id",id);if(error){alert("Failed to delete equipment.");throw error;}}),
    addAgreement:withTableSync("service_agreements",async(agr)=>{const{data:existing}=await sb().from("service_agreements").select("agreement_num");const num=genAgreementNum(existing||[]);const{error}=await sb().from("service_agreements").insert({...agr,agreement_num:num,created_by:appUser.name});if(error){alert("Failed to add agreement.");throw error;}}),
    updateAgreement:withTableSync("service_agreements",async(agr)=>{const{id,...rest}=agr;rest.updated_at=new Date().toISOString();const{error}=await sb().from("service_agreements").update(rest).eq("id",id);if(error){alert("Failed to update agreement.");throw error;}}),
    deleteAgreement:withTableSync("service_agreements",async(id)=>{const{error}=await sb().from("service_agreements").delete().eq("id",id);if(error){alert("Failed to delete agreement.");throw error;}}),
    addAgreementTier:withTableSync("agreement_tiers",async(t)=>{const{error}=await sb().from("agreement_tiers").insert(t);if(error){alert("Failed to add tier.");throw error;}}),
    updateAgreementTier:withTableSync("agreement_tiers",async(t)=>{const{id,...rest}=t;const{error}=await sb().from("agreement_tiers").update(rest).eq("id",id);if(error){alert("Failed to update tier.");throw error;}}),
    deleteAgreementTier:withTableSync("agreement_tiers",async(id)=>{const{error}=await sb().from("agreement_tiers").delete().eq("id",id);if(error){alert("Failed to delete tier.");throw error;}}),
    rejectDraft:withTableSync("wo_drafts",async(draftId,reason)=>{
      const{error}=await sb().from("wo_drafts").update({status:"rejected",reviewed_by:appUser.id,reviewed_at:new Date().toISOString(),reject_reason:reason||null}).eq("id",draftId);if(error){alert("Failed to reject draft.");throw error;}
    }),
    // ── RFQs (Request for Quotation) ──────────────────────────
    createRFQ:withTableSync("rfqs",async({rfq,items,specs})=>{
      const ref=genRfqRef(data.rfqs,rfq.descriptor);
      const{descriptor,...rfqFields}=rfq;
      const{data:inserted,error}=await sb().from("rfqs").insert({...rfqFields,rfq_ref:ref,status:"draft",created_by:appUser.id}).select("id").single();
      if(error){alert("Failed to create RFQ. "+error.message);throw error;}
    ...
    updateRFQ:withTableSync("rfqs",async({id,rfq,items,specs})=>{
      const{descriptor,...rfqFields}=rfq||{};
      const{error}=await sb().from("rfqs").update(rfqFields).eq("id",id);
