...
const getRoles=()=>({admin:{label:"Admin",color:B.red,grad:`linear-gradient(135deg,${B.red},#C0392B)`},manager:{label:"Manager",color:B.green,grad:`linear-gradient(135deg,${B.green},#1A9A73)`},technician:{label:"Technician",color:B.cyan,grad:`linear-gradient(135deg,${B.cyan},${B.cyanDark})`}});
let ROLES=getRoles();
...
const autoCorrect=(text)=>{if(!text||typeof text!=="string")return text;let t=text;
  t=t.replace(/\s{2,}/g," ");
  t=t.replace(/\n\s*\n\s*\n/g,"\n\n");
...
const IS=new Proxy({},{get:(_,p)=>_IS()[p],ownKeys:()=>Object.keys(_IS()),getOwnPropertyDescriptor:(_,p)=>({value:_IS()[p],enumerable:true,configurable:true})});
const LS=new Proxy({},{get:(_,p)=>_LS()[p],ownKeys:()=>Object.keys(_LS()),getOwnPropertyDescriptor:(_,p)=>({value:_LS()[p],enumerable:true,configurable:true})});
const BP=new Proxy({},{get:(_,p)=>_BP()[p],ownKeys:()=>Object.keys(_BP()),getOwnPropertyDescriptor:(_,p)=>({value:_BP()[p],enumerable:true,configurable:true})});
const BS=new Proxy({},{get:(_,p)=>_BS()[p],ownKeys:()=>Object.keys(_BS()),getOwnPropertyDescriptor:(_,p)=>({value:_BS()[p],enumerable:true,configurable:true})});
...
function genProjectPO(list){const n=new Date(),pfx="PPO-"+String(n.getFullYear()).slice(2)+String(n.getMonth()+1).padStart(2,"0");const mx=list.filter(p=>p.po_id&&p.po_id.startsWith(pfx)).reduce((m,p)=>{const s=parseInt(p.po_id.slice(pfx.length));return s>m?s:m;},0);return pfx+String(mx+1).padStart(2,"0");}
...
function Badge({color,children}){return <span style={{display:"inline-block",padding:"4px 10px",borderRadius:20,background:color+"18",color,fontSize:11,fontWeight:700,textTransform:"uppercase",fontFamily:F,letterSpacing:0.3,border:"1px solid "+color+"30",animation:"badgePop .2s ease-out"}}>{children}</span>;}
...
function ConfirmDialog({message,onConfirm,onCancel,confirmLabel,danger}){return<Modal title="Confirm" onClose={onCancel}><div style={{textAlign:"center",padding:"8px 0"}}><div style={{fontSize:32,marginBottom:8}}>{danger?"⚠️":"❓"}</div><div style={{fontSize:13,color:B.text,marginBottom:16,lineHeight:1.5}}>{message}</div><div style={{display:"flex",gap:8}}><button onClick={onCancel} style={{..._BS(),flex:1}}>Cancel</button><button onClick={onConfirm} style={{..._BP(),flex:1,background:danger?B.red:B.cyan}}>{confirmLabel||"Confirm"}</button></div></div></Modal>;}
...
function Shell({user,onLogout,children,tab,setTab,tabs,syncing,notifications,onMarkRead,onQuickApprovePO,onQuickRejectPO,onNavigateWO,onRefresh}){
  ...
  const toggleTheme=()=>{const t=theme==="dark"?"light":"dark";setTheme(t);ROLES=getRoles();setThemeState(t);};
...
// ═══════════════════════════════════════════
const WF_TRIGGERS=[{key:"wo_created",label:"Work Order Created",icon:"📋",fields:["customer","priority","wo_type","assignee"]},{key:"wo_completed",label:"WO Completed",icon:"✅",fields:["customer","priority","wo_type","assignee"]},{key:"wo_status_changed",label:"WO Status Changed",icon:"🔄",fields:["customer","priority","status","assignee"]},{key:"invoice_sent",label:"Invoice Sent",icon:"📧",fields:["customer","amount","status"]},{key:"invoice_overdue",label:"Invoice Overdue (30d)",icon:"⚠️",fields:["customer","amount"]},{key:"po_requested",label:"PO Requested",icon:"📄",fields:["amount","customer"]},{key:"po_approved",label:"PO Approved",icon:"✅",fields:["amount","customer"]},{key:"feedback_received",label:"Feedback Received",icon:"⭐",fields:["customer_name","star_rating","nps_score"]}];
...
// ═══════════════════════════════════════════
// FEEDBACK FORM — Public page (no auth)
// ═══════════════════════════════════════════
