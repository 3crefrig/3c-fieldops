...
function App(){
  ...
  const withTableSync=(table,fn)=>async(...args)=>{setSyncing(true);try{await fn(...args);await reloadTable(table);}catch(e){console.error(table+" operation failed:",e);if(e?.message)alert("Operation failed: "+e.message);throw e;}finally{setSyncing(false);}};
  ...
  const actions={
    ...
    updateTime:withTableSync("time_entries",async(te)=>{if(te.description)te.description=autoCorrect(te.description);const{id,...rest}=te;const{error}=await sb().from("time_entries").update(rest).eq("id",id);if(error){alert("Failed to update time entry.");throw error;}}),
    deleteTime:withTableSync("time_entries",async(id)=>{const{error}=await sb().from("time_entries").delete().eq("id",id);if(error){alert("Failed to delete time entry.");throw error;}}),
