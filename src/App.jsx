...
import { sb, SUPABASE_URL, SUPABASE_ANON_KEY, USER_COLS, WO_COLS, B, F, autoCorrect, genPO, genProjectPO, genRfqRef, GlobalStyles, setProfanityHandler, fmtHours, isInvoiceExcludedCustomer, woReadyToInvoice, fnFetch, getCustomerTiers, getPartsMarkup, todayLocal, localDateStr, genAgreementNum, nextInvoiceNumDB} from "./shared";
...
function App(){
  ...
  const loadData=useCallback(async()=>{try{const client=sb();if(!client)return;
    ...
    [wos,pos,time,photos,users,schedule,templates,notifs,customers,emailTemplates,projects,woDrafts,invoices,feedbackData,equipmentData,agreementsData,agreementTiersData,lineItemsData,rfqsData,rfqItemsData,rfqSpecsData,poTicketsData,poTicketItemsData,vendorBillsData,vendorBillItemsData].forEach((r,i)=>{if(r.error)console.warn("loadData query "+i+" failed:",r.error.message);});
