...
function App(){
  ...
  const actions={
    ...
    createPO:withSync(async(po)=>{if(po.amount&&parseFloat(po.amount)<0){alert("PO amount cannot be negative.");return;}if(po.description)po.description=autoCorrect(po.description);if(po.notes)po.notes=autoCorrect(po.notes);const{data:all}=await sb().from("purchase_orders").select("po_id");const id=genPO(all||[]);await sb().from("purchase_orders").insert({...po,po_id:id,requested_by:appUser.name,status:"pending"});await notify("po_requested","PO Requested",id+" — $"+po.amount+" by "+appUser.name,"manager");evaluateTriggers("po_requested",{...po,po_id:id});}),
    ...
    addAgreement:withSync(async(agr)=>{const{data:existing}=await sb().from("service_agreements").select("agreement_num");const num=genAgreementNum(existing||[]);await sb().from("service_agreements").insert({...agr,agreement_num:num,created_by:appUser.name});}),
