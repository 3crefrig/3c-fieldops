...
function App(){
  ...
  const actions={
    ...
    createPO:withTableSync("purchase_orders",async(po)=>{if(po.amount&&parseFloat(po.amount)<0){alert("PO amount cannot be negative.");return;}if(po.description)po.description=autoCorrect(po.description);if(po.notes)po.notes=autoCorrect(po.notes);const{data:all}=await sb().from("purchase_orders").select("po_id");const id=genPO(all||[]);await sb().from("purchase_orders").insert({...po,po_id:id,requested_by:appUser.name,status:"pending"});notify("po_requested","PO Requested",id+" — $"+po.amount+" by "+appUser.name,"manager");evaluateTriggers("po_requested",{...po,po_id:id});}),
    ...
    addAgreement:withTableSync("service_agreements",async(agr)=>{const{data:existing}=await sb().from("service_agreements").select("agreement_num");const num=genAgreementNum(existing||[]);await sb().from("service_agreements").insert({...agr,agreement_num:num,created_by:appUser.name});}),
    updateAgreement:withTableSync("service_agreements",async(agr)=>{const{id,...rest}=agr;rest.updated_at=new Date().toISOString();await sb().from("service_agreements").update(rest).eq("id",id);}),
    deleteAgreement:withTableSync("service_agreements",async(id)=>{await sb().from("service_agreements").delete().eq("id",id);}),
    addAgreementTier:withTableSync("agreement_tiers",async(t)=>{await sb().from("agreement_tiers").insert(t);}),
    updateAgreementTier:withTableSync("agreement_tiers",async(t)=>{const{id,...rest}=t;await sb().from("agreement_tiers").update(rest).eq("id",id);}),
    deleteAgreementTier:withTableSync("agreement_tiers",async(id)=>{await sb().from("agreement_tiers").delete().eq("id",id);}),
