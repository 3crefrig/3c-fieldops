...
function App(){
  ...
  useEffect(()=>{if(!authUser)return;const client=sb();
    ...
    const checkInvoiceBacklog=async()=>{const today=new Date().toISOString().slice(0,10);const{data:done}=await client.from("work_orders").select("id,wo_id,customer,status,invoiced,project_id").eq("status","completed").eq("invoiced",false).is("project_id",null);if(!done||done.length===0)return;const{data:custs}=await client.from("customers").select("name,auto_invoice");const autoNames=new Set((custs||[]).filter(c=>c.auto_invoice).map(c=>c.name));const ready=done.filter(w=>!isInvoiceExcludedCustomer(w.customer)&&!autoNames.has(w.customer));for(const w of ready){const{data:existing}=await client.from("notifications").select("id").eq("type","wo_needs_invoice").ilike("message","%"+w.wo_id+"%").gte("created_at",today);if(!existing||existing.length===0)await client.from("notifications").insert({type:"wo_needs_invoice",title:"Ready to Invoice",message:(w.wo_id||"")+" — "+(w.customer||"customer")+" is complete and needs invoicing",for_role:"manager"});}};
    checkInvoiceBacklog();
