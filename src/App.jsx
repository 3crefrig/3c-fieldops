...
function App(){
  ...
  const actions={
    ...
    createPO:withTableSync("purchase_orders",async(po)=>{if(po.amount&&parseFloat(po.amount)<0){alert("PO amount cannot be negative.");return;}if(po.description)po.description=autoCorrect(po.description);if(po.notes)po.notes=autoCorrect(po.notes);const wo=po.wo_id?data.wos.find(w=>w.id===po.wo_id):null;const projId=wo?.project_id||null;const{data:all}=await sb().from("purchase_orders").select("po_id");const id=projId?genProjectPO(all||[]):genPO(all||[]);const payload={...po,po_id:id,requested_by:appUser.name,status:"pending"};if(projId)payload.project_id=projId;const{error}=await sb().from("purchase_orders").insert(payload);if(error){console.error("createPO error:",error);alert("Failed to create PO. Please try again.");throw error;}notify("po_requested","PO Requested",id+" — $"+po.amount+" by "+appUser.name,"manager");evaluateTriggers("po_requested",{...po,po_id:id});}),
    ...
    deleteInvoice:withTableSync("invoices",async(inv)=>{const{error}=await sb().from("invoices").delete().eq("id",inv.id);if(error){alert("Failed to delete invoice.");throw error;}}),
