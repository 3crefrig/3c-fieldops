  ...
  return(<div><Toast msg={toast}/>
    ...
    {tab==="overview"&&<div>
      ...
      {isMgr&&<Card style={{padding:14,marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={LS}>Budget & Financials</span>{!editBudget?<button onClick={()=>{setLocalBudget(project.budget||0);setEditBudget(true);}} style={{background:"none",border:"none",color:B.cyan,fontSize:10,cursor:"pointer",fontFamily:F}}>Edit Budget</button>:<div style={{display:"flex",gap:4}}><input value={localBudget} onChange={e=>setLocalBudget(parseFloat(e.target.value)||0)} type="number" style={{...IS,width:100,padding:"4px 8px",fontSize:12,fontFamily:M}}/><button onClick={async()=>{await onUpdate({...project,budget:localBudget});setEditBudget(false);msg("Budget updated");}} style={{background:"none",border:"none",color:B.green,fontSize:11,cursor:"pointer",fontFamily:F}}>Save</button></div>}</div>
        ...
        {isAdmin&&chambers.length>0&&<div style={{marginTop:12}}><div style={{fontSize:10,fontWeight:600,color:B.textDim,marginBottom:6}}>Cost by Chamber</div>{chambers.map(ch=>{const chWOs=pWOs.filter(w=>w.building===ch.name||w.location===ch.name);const chMat=(allPOs||[]).filter(p=>chWOs.some(w=>w.id===p.wo_id)&&p.status==="approved").reduce((s,p)=>s+parseFloat(p.amount||0),0);const chTime=chWOs.flatMap(w=>(allTime||[]).filter(t=>t.wo_id===w.id));const chLabor=chTime.reduce((s,t)=>s+parseFloat(t.hours||0)*getRate(t.technician,"cost"),0);const chTotal=chMat+chLabor;const chPct=project.budget>0?chTotal/project.budget*100:0;return<div key={ch.id} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:B.textMuted}}><span>{ch.name}</span><span style={{fontFamily:M}}>{"$"+chTotal.toLocaleString()+" ("+chPct.toFixed(0)+"%)"}</span></div><div style={{background:B.bg,borderRadius:3,height:6,marginTop:2,overflow:"hidden"}}><div style={{width:Math.min(100,chPct)+"%",height:"100%",background:B.cyan,borderRadius:3}}/></div></div>})}</div>}
