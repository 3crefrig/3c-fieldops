...
function BillingExport({wos,pos,timeEntries,customers,emailTemplates,currentUser}){
  ...
  const getTimesheetRows=()=>{const rows=[];let totalHrs=0;completed.forEach(wo=>{const woNum=wo.customer_wo||wo.wo_id;const woTime=timeEntries.filter(t=>t.wo_id===wo.id);const woDesc=wo.work_performed||wo.notes&&wo.notes!=="No details."&&wo.notes||wo.title;if(woTime.length>0){woTime.forEach(te=>{const h=parseFloat(te.hours||0);const desc=wo.work_performed||te.description||woDesc;rows.push({date:te.logged_date||wo.date_completed||"",building:wo.building||"",room:wo.location||"",wo_num:woNum,hours:h,desc});totalHrs+=h;});}else{const h=timeEntries.filter(t=>t.wo_id===wo.id).reduce((s,t)=>s+parseFloat(t.hours||0),0);rows.push({date:wo.date_completed||"",building:wo.building||"",room:wo.location||"",wo_num:woNum,hours:h,desc:woDesc});totalHrs+=h;}});return{rows,totalHrs};};
