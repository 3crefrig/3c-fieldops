...
function Reports({wos,pos,timeEntries,users,customers,invoices}){
  ...
  const exportPDF=async()=>{const{jsPDF}=await import("jspdf");const doc=new jsPDF();doc.setFontSize(18);doc.text("3C FieldOps Report",14,20);doc.setFontSize(10);doc.text("Period: "+(from||"All time")+" to "+(to||"Present"),14,28);doc.setFontSize(11);let y=38;doc.text("Completed WOs: "+completed.length,14,y);y+=7;doc.text("Total Hours: "+fmtHours(totalHours),14,y);y+=7;doc.text("PO Spend: $"+totalPOSpend.toLocaleString(),14,y);y+=7;doc.text("Avg Job Duration: "+fmtHours(avgJobDuration),14,y);y+=7;doc.text("PM / CM: "+pmCount+" / "+cmCount,14,y);y+=14;doc.setFontSize(12);doc.text("Technician Performance",14,y);y+=8;doc.setFontSize(9);techs.forEach(t=>{const h=fTime.filter(e=>e.technician===t.name).reduce((s,e)=>s+parseFloat(e.hours||0),0);const done=completed.filter(w=>w.assignee===t.name).length;if(y>270){doc.addPage();y=20;}doc.text(t.name+": "+fmtHours(h)+", "+done+" WOs completed",18,y);y+=6;});y+=8;if(y>250){doc.addPage();y=20;}doc.setFontSize(12);doc.text("Completed Work Orders",14,y);y+=8;doc.setFontSize(8);completed.slice(0,40).forEach(w=>{const hrs=calcWOHours(w.id,fTime);if(y>275){doc.addPage();y=20;}doc.text((w.wo_id||"")+" - "+(w.title||"").slice(0,40)+" | "+(w.customer||"")+" | "+fmtHours(hrs),18,y);y+=5;});doc.save("3C_Report_"+(from||"all")+"_to_"+(to||"now")+".pdf");};
