...
import { B, F, M, IS, LS, BP, BS, ROLES, calcWOHours, fmtHours, getPartsMarkup, getCustomerTiers , todayLocal, localDateStr, importRetry} from "../shared";
...
function Reports({wos,pos,timeEntries,users,customers,invoices}){
  ...
  const exportPDF=async()=>{const{jsPDF}=await importRetry(()=>import("jspdf"));const doc=new jsPDF();doc.setFontSize(18);doc.text("3C FieldOps Report",14,20);doc.setFontSize(10);doc.text("Period: "+(from||"All time")+" to "+(to||"Present"),14,28);doc.setFontSize(11);let y=38;doc.text("Completed WOs: "+completed.length,14,y);y+=7;doc.text("Total Hours: "+fmtHours(totalHours),14,y);y+=7;doc.text("PO Spend: $"+totalPOSpend.toLocaleString(),14,y);y+=7;doc.text("Avg Job Duration: "+fmtHours(avgJobDuration),14,y);y+=7;doc.text("PM / CM: "+pmCount+" / "+cmCount,14,y);y+=14;doc.setFontSize(12);doc.text("Technician Performance",14,y);y+=8;doc.setFontSize(9);techs.forEach(t=>{const h=fTime.filter(e=>e.technician===t.name).reduce((s,e)=>s+parseFloat(e.hours||0),0);const done=completed.filter(w=>w.assignee===t.name).length;if(y>270){doc.addPage();y=20;}doc.text(t.name+": "+fmtHours(h)+", "+done+" WOs completed",18,y);y+=6;});y+=8;if(y>250){doc.addPage();y=20;}doc.setFontSize(12);doc.text("Completed Work Orders",14,y);y+=8;doc.setFontSize(8);completed.slice(0,40).forEach(w=>{const hrs=calcWOHours(w.id,fTime);if(y>275){doc.addPage();y=20;}doc.text((w.wo_id||"")+" - "+(w.title||"").slice(0,40)+" | "+(w.customer||"")+" | "+fmtHours(hrs),18,y);y+=5;});doc.save("3C_Report_"+(from||"all")+"_to_"+(to||"now")+".pdf");};

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}><h3 style={{margin:0,fontSize:15,fontWeight:700,color:B.text}}>Reports & KPIs</h3><button data-tip="Download this period as a spreadsheet — one row per completed job with hours and PO spend." onClick={exportCSV} style={{...BS,padding:"5px 12px",fontSize:11}}>Export CSV</button><button data-tip="A one-page PDF summary of this period — handy for meetings." onClick={exportPDF} style={{...BS,padding:"5px 12px",fontSize:11}}>Export PDF</button></div>
