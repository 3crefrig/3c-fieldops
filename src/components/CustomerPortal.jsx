...
import { sb, B, F, M, SC, SL, fmtHours } from "../shared";
...
function CustomerPortal({customerSlug}){
  ...
  return(<div style={{minHeight:"100vh",background:B.bg,fontFamily:F,color:B.text}}>
    ...
    <div style={{maxWidth:900,margin:"0 auto",padding:20}}>
      <h2 style={{fontSize:20,fontWeight:600,marginBottom:4}}>{data.name}</h2>
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}><StatCard label="Active" value={active.length} icon="wrench" color={B.cyan}/><StatCard label="Completed" value={done.length} icon="✓" color={B.green}/><StatCard label="Total Hours" value={fmtHours(totalHrs)} icon="clock" color={B.orange}/></div>
      {active.length>0&&<><h3 style={{fontSize:14,fontWeight:600,color:B.text,marginBottom:8}}>Active Work Orders</h3>{active.map(wo=><Card key={wo.id} style={{padding:"12px 16px",marginBottom:6}}>
      ...
      {done.length>0&&<><h3 style={{fontSize:14,fontWeight:600,color:B.text,marginBottom:8,marginTop:16}}>Completed</h3>{done.slice(0,20).map(wo=>{const hrs=data.time.filter(t=>t.wo_id===wo.id).reduce((s,t)=>s+parseFloat(t.hours||0),0);return<Card key={wo.id} style={{padding:"12px 16px",marginBottom:6}}>
