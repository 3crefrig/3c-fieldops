...
export function StatCard({label,value,icon,color,delta}){return <Card style={{flex:"1 1 130px",minWidth:130,borderLeft:"3px solid "+color,padding:"16px 18px"}}><div style={{fontSize:10,color:B.textDim,fontWeight:600,letterSpacing:.4,textTransform:"uppercase",marginBottom:6}}>{label}</div><div style={{fontSize:26,fontWeight:700,color,fontFamily:M,lineHeight:1,letterSpacing:-0.5}}>{value}</div>{delta!=null&&<div style={{fontSize:10,fontWeight:700,fontFamily:M,marginTop:6,color:delta>=0?B.green:B.red}}>{delta>=0?"↑":"↓"}{Math.abs(delta)}% <span style={{color:B.textDim,fontWeight:400,fontFamily:F}}>vs prior</span></div>}</Card>;}
