...
function UserMgmt({users,onAddUser,onUpdateUser,onDeleteUser,onSendOnboarding,cur}){
  const[showForm,setShowForm]=useState(false),[editing,setEditing]=useState(null),[search,setSearch]=useState(""),[rf,setRf]=useState("all"),[toast,setToast]=useState(""),[sending,setSending]=useState(null);
  const msg=m=>{setToast(m);setTimeout(()=>setToast(""),2500);};
  const onboard=async(u)=>{if(sending)return;if(!u.email){msg("No email on file for "+u.name);return;}if(!window.confirm("Send the onboarding email to "+u.name+" ("+u.email+")?"))return;setSending(u.id);const r=await onSendOnboarding(u);setSending(null);if(r?.ok)msg("Onboarding email sent to "+u.name);};
  ...
  return(<div><Toast msg={toast}/>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>{filtered.map(u=><Card key={u.id} style={{padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",opacity:u.active===false?0.6:1}}><div><div style={{fontSize:14,fontWeight:700,color:B.text}}>{u.name}{u.active===false&&<span style={{fontSize:10,color:B.red,fontWeight:600,marginLeft:8,background:B.red+"18",padding:"2px 6px",borderRadius:4}}>INACTIVE</span>}</div><div style={{fontSize:11,color:B.textDim}}>{u.email} · <Badge color={ROLES[u.role]?ROLES[u.role].color:B.textDim}>{u.role}</Badge></div></div><div style={{display:"flex",alignItems:"center",gap:6}}>{u.id!==cur.id&&<button onClick={async()=>{await onUpdateUser({...u,active:u.active===false?true:false});msg(u.active===false?"Activated "+u.name:"Deactivated "+u.name);}} style={{background:"none",border:"none",color:u.active===false?B.green:B.orange,fontSize:10,cursor:"pointer",fontWeight:600}}>{u.active===false?"Activate":"Deactivate"}</button>}{onSendOnboarding&&<button data-tip="Re-send the welcome email — sign-in steps, add-to-home-screen, and alerts setup for this person." onClick={()=>onboard(u)} disabled={sending===u.id} title="Send onboarding email" style={{background:"none",border:"none",color:B.green,fontSize:11,cursor:"pointer",fontWeight:600,opacity:sending===u.id?.5:1}}>{sending===u.id?"Sending…":"Onboard"}</button>}<button onClick={()=>{setEditing(u);setShowForm(true)}} style={{background:"none",border:"none",color:B.cyan,fontSize:11,cursor:"pointer"}}>Edit</button>{u.id!==cur.id&&<button onClick={async()=>{await onDeleteUser(u.id);msg("Deleted "+u.name)}} style={{background:"none",border:"none",color:B.red,fontSize:11,cursor:"pointer"}}>×</button>}</div></Card>)}</div>
