...
function POReqModal({wo,pos,onCreatePO,onClose,userName,userRole}){
  ...
  return(<Modal title="Purchase Order" onClose={onClose} wide>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div><input ref={scanReceiptRef} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={handleScanReceipt}/><button onClick={()=>scanReceiptRef.current?.click()} disabled={scanningReceipt} type="button" style={{...BS,width:"100%",padding:"10px 14px",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:scanningReceipt?.6:1}}>{scanningReceipt?"Scanning...":"📷 Scan Receipt"}</button>{scanningReceipt&&<div style={{fontSize:11,color:B.cyan,marginTop:4,textAlign:"center"}}>AI is reading the receipt...</div>}</div>
