    ...
    {tab==="today"&&<>{(()=>{const noTimeToday=myActive.filter(o=>o.status==="in_progress"&&!D.time.some(t=>t.wo_id===o.id&&t.logged_date===todayStr));const isAfternoon=new Date().getHours()>=15;return<>
      ...
      {(()=>{const thirtyDays=new Date(Date.now()-30*86400000);const myCompleted30=my.filter(o=>o.status==="completed"&&o.date_completed&&new Date(o.date_completed)>=thirtyDays);
        ...
        return <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"16px 18px",background:B.surface,borderRadius:12,border:"1px solid "+B.border,boxShadow:"0 1px 0 rgba(26,26,26,.05)",boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,marginBottom:6}}>Fix Rate (30d)</div><div className="stat-value" style={{fontSize:24,fontWeight:650,fontFamily:F,lineHeight:1,letterSpacing:-0.4,color:ftfr>=85?B.green:ftfr>=70?B.orange:B.red}}>{ftfr}%</div></div>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"16px 18px",background:B.surface,borderRadius:12,border:"1px solid "+B.border,boxShadow:"0 1px 0 rgba(26,26,26,.05)",boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,marginBottom:6}}>Week Utilization</div><div className="stat-value" style={{fontSize:24,fontWeight:650,fontFamily:F,lineHeight:1,letterSpacing:-0.4,color:util>=80?B.green:util>=60?B.orange:B.red}}>{util}%</div></div>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"16px 18px",background:B.surface,borderRadius:12,border:"1px solid "+B.border,boxShadow:"0 1px 0 rgba(26,26,26,.05)",boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,marginBottom:6}}>Week Hours</div><div className="stat-value" style={{fontSize:24,fontWeight:650,fontFamily:F,lineHeight:1,letterSpacing:-0.4,color:B.text}}>{fmtHours(weekHrs)}</div></div>
        </div>;})()}
