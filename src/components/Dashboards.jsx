...
function RepeatFailures({wos,pos,equipment}){
  ...
  return(<Card style={{padding:"14px 18px",marginBottom:12,borderLeft:"3px solid "+B.red}}>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><span style={{fontSize:16}}>🔁</span><span style={{fontSize:13,fontWeight:700,color:B.text}}>Repeat Failure Alerts</span><span style={{fontSize:10,color:B.textDim}}>(90 days)</span></div>
    ...
    {tab==="today"&&<>{(()=>{const noTimeToday=myActive.filter(o=>o.status==="in_progress"&&!D.time.some(t=>t.wo_id===o.id&&t.logged_date===todayStr));const isAfternoon=new Date().getHours()>=15;return<>
      ...
      {(()=>{const thirtyDays=new Date(Date.now()-30*86400000);const myCompleted30=my.filter(o=>o.status==="completed"&&o.date_completed&&new Date(o.date_completed)>=thirtyDays);
        ...
        return <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
          <Card style={{flex:"1 1 100px",minWidth:100,padding:"10px 14px"}}><div style={{fontSize:10,color:B.textDim,fontWeight:600,letterSpacing:.4,textTransform:"uppercase",marginBottom:4}}>Fix Rate (30d)</div><div style={{fontSize:20,fontWeight:700,fontFamily:M,color:ftfr>=85?B.green:ftfr>=70?B.orange:B.red}}>{ftfr}%</div></Card>
          <Card style={{flex:"1 1 100px",minWidth:100,padding:"10px 14px"}}><div style={{fontSize:10,color:B.textDim,fontWeight:600,letterSpacing:.4,textTransform:"uppercase",marginBottom:4}}>Week Utilization</div><div style={{fontSize:20,fontWeight:700,fontFamily:M,color:util>=80?B.green:util>=60?B.orange:B.red}}>{util}%</div></Card>
          <Card style={{flex:"1 1 100px",minWidth:100,padding:"10px 14px"}}><div style={{fontSize:10,color:B.textDim,fontWeight:600,letterSpacing:.4,textTransform:"uppercase",marginBottom:4}}>Week Hours</div><div style={{fontSize:20,fontWeight:700,fontFamily:M,color:B.cyan}}>{fmtHours(weekHrs)}</div></Card>
        </div>;})()}
