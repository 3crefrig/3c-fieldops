  ...
  return(<Card style={{padding:"14px 18px",marginBottom:12,borderLeft:"3px solid "+B.red}}>
    ...
    return<div key={i} style={{padding:"8px 0",borderTop:i>0?"1px solid "+B.border:"none"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <span style={{fontFamily:M,fontSize:12,fontWeight:800,color:B.red}}>{g.wos.length} CMs</span>
      </div>
...
function TechDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  const techTabs=["today","planner","orders","time","equipment","rfqs","calendar","projects","kb"];
  ...
  return(<Shell user={user} onLogout={onLogout} tab={tab} setTab={setTab} syncing={syncing} offlineQueueCount={offlineQueueCount} notifications={D.notifs} onMarkRead={A.markRead} onQuickApprovePO={A.quickApprovePO} onQuickRejectPO={A.quickRejectPO} onNavigateWO={(woId)=>{setTab("orders");if(woId)setNavWOId(woId);}} onRefresh={A.loadData} searchData={{wos:D.wos,pos:D.pos,customers:D.customers,equipment:D.equipment,projects:D.projects}} tabs={[{key:"today",label:"My Day",icon:"📍"},{key:"planner",label:"Week Plan",icon:"🗓"},{key:"orders",label:"All Orders",icon:"📋"},{key:"time",label:"Hours",icon:"⏱"},{key:"equipment",label:"Equipment",icon:"🔧"},{key:"rfqs",label:"RFQs",icon:"📨"},{key:"calendar",label:"Calendar",icon:"📅"},{key:"projects",label:"Projects",icon:"🏗️"},{key:"kb",label:"Knowledge",icon:"📖"}]}>
...
function MgrDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  const mgrTabs=["overview","inbox","orders","planner","pos","rfqs","reports","billing","invoices","feedback","agreements","equipment","team","customers","users","calendar","projects","kb"];
...
function AdminDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  const adminTabs=["overview","inbox","orders","planner","pos","rfqs","reports","billing","invoices","feedback","proposals","agreements","recurring","equipment","customers","users","settings","calendar","projects","kb"];
