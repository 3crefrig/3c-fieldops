...
const PriceBook=lazyNamed(()=>import("./PriceBook"),"PriceBook");
...
// Newly split (2026-08-04): heavy tab-only screens, none on the default landing.
const InvoiceDashboard=lazyNamed(()=>import("./Invoices"),"InvoiceDashboard");
const POMgmt=lazyNamed(()=>import("./PurchaseOrders"),"POMgmt");
const Projects=lazyNamed(()=>import("./Projects"),"Projects");
const EquipmentDashboard=lazyNamed(()=>import("./Equipment"),"EquipmentDashboard");
const CompanyCalendar=lazyNamed(()=>import("./Calendar"),"CompanyCalendar");
const CustomerMgmt=lazyNamed(()=>import("./Customers"),"CustomerMgmt");
const UserMgmt=lazyNamed(()=>import("./Users"),"UserMgmt");
const ServiceRequests=lazyNamed(()=>import("./ServiceRequests"),"ServiceRequests");
const DayPlanner=lazyNamed(()=>import("./DayPlanner"),"DayPlanner");
const BillingExport=lazyNamed(()=>import("./Billing"),"BillingExport");
const RecurringPM=lazyNamed(()=>import("./RecurringPM"),"RecurringPM");
...
function RepeatFailures({wos,pos,equipment}){
  ...
  const flagged=Object.values(groups).filter(g=>g.wos.length>=3).sort((a,b)=>b.wos.length-a.wos.length);
  if(flagged.length===0)return null;
  return(<Card style={{padding:"14px 18px",marginBottom:12}}>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><span style={{display:"inline-flex",color:B.red}}><Icon name="repeat" size={15}/></span><span style={{fontSize:13,fontWeight:700,color:B.text}}>Repeat Failure Alerts</span><span style={{fontSize:10,color:B.textDim}}>(90 days)</span></div>
    {flagged.map((g,i)=>{const eq=g.eqId&&(equipment||[]).find(e=>e.id===g.eqId);const woIds=g.wos.map(w=>w.id);const spend=(pos||[]).filter(p=>woIds.includes(p.wo_id)&&p.status==="approved").reduce((s,p)=>s+parseFloat(p.amount||0),0);const rec=g.wos.length>=5?"Recommend replacement evaluation":spend>500?"High repair cost — consider replacement":"Schedule comprehensive diagnostic";const color=g.wos.length>=5?B.red:spend>500?B.orange:B.cyan;
    return<div key={i} style={{padding:"8px 0",borderTop:i>0?"1px solid "+B.border:"none"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><span style={{fontSize:12,fontWeight:700,color:B.text}}>{eq?eq.model+" — ":""}{g.location||"Unknown"}{g.building?" / "+g.building:""}</span><span style={{fontSize:10,color:B.textDim,marginLeft:6}}>{g.customer}</span></div>
        <span style={{fontFamily:M,fontSize:12,fontWeight:700,color:B.red}}>{g.wos.length} CMs</span>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4}}>
        <span style={{fontSize:11,color,fontWeight:600}}>{rec}</span>
        {spend>0&&<span style={{fontSize:10,fontFamily:M,color:B.orange}}>{"$"+spend.toLocaleString()+" in parts"}</span>}
      </div>
    </div>})}
  </Card>);
}
...
function QuickLogFAB({wos,userName,onAddTime}){
  ...
  return(<>
    <button data-tour="quick-log" onClick={()=>{setOpen(true);haptic(30);}} title="Quick log time" style={{position:"fixed",bottom:"max(84px, calc(66px + env(safe-area-inset-bottom)))",right:14,width:52,height:52,borderRadius:"50%",background:B.cyan,border:"none",color:B.bg,cursor:"pointer",boxShadow:"0 4px 14px rgba(0,0,0,0.28)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:150}}><Icon name="clock" size={24}/></button>
...
function TechDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  ...
  return(<Shell user={user} onLogout={onLogout} tab={tab} setTab={setTab} syncing={syncing} offlineQueueCount={offlineQueueCount} notifications={visibleNotifs(D.notifs,user.role)} onMarkRead={A.markRead} onQuickApprovePO={A.quickApprovePO} onQuickRejectPO={A.quickRejectPO} onNavigateWO={(woId)=>{setTab("orders");if(woId)setNavWOId(woId);}} onRefresh={A.loadData} searchData={{wos:D.wos,pos:D.pos,customers:D.customers,equipment:D.equipment,projects:D.projects}} tabs={[{key:"today",label:"My Day",icon:"pin"},{key:"planner",label:"Week Plan",icon:"calendar"},{key:"orders",label:"All Orders",icon:"clipboard"},{key:"time",label:"Hours",icon:"clock"},{key:"equipment",label:"Equipment",icon:"wrench"},{key:"rfqs",label:"RFQs",icon:"mail"},{key:"calendar",label:"Calendar",icon:"calendar"},{key:"projects",label:"Projects",icon:"building"},{key:"kb",label:"Knowledge",icon:"book"},{key:"guide",label:"Guide",icon:"book"}]}>
    {tab==="calendar"&&<CompanyCalendar userRole={user.role} wos={D.wos} userName={user.name} time={D.time} schedule={D.schedule||[]} users={D.users||[]}/>}
    {tab==="today"&&<>{(()=>{const noTimeToday=myActive.filter(o=>o.status==="in_progress"&&!D.time.some(t=>t.wo_id===o.id&&t.logged_date===todayStr));const isAfternoon=new Date().getHours()>=15;return<>
      ...
      <div style={{display:"flex",gap:10,marginBottom:10,flexWrap:"wrap"}}>
        <StatCard label="Active Jobs" value={myActive.length} icon="wrench" color={B.cyan}/>
        <StatCard label="Today's Hours" value={fmtHours(todayHours)} icon="clock" color={B.green}/>
      ...
      {(()=>{const thirtyDays=new Date(Date.now()-30*86400000);const myCompleted30=my.filter(o=>o.status==="completed"&&o.date_completed&&new Date(o.date_completed)>=thirtyDays);
        ...
        return <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"14px 16px",background:B.surface,borderRadius:8,border:"1px solid "+B.border,boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,letterSpacing:0.3,textTransform:"uppercase",marginBottom:6}}>Fix Rate (30d)</div><div className="stat-value" style={{fontSize:22,fontWeight:700,fontFamily:M,lineHeight:1,letterSpacing:-0.5,fontVariantNumeric:"tabular-nums",color:ftfr>=85?B.green:ftfr>=70?B.orange:B.red}}>{ftfr}%</div></div>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"14px 16px",background:B.surface,borderRadius:8,border:"1px solid "+B.border,boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,letterSpacing:0.3,textTransform:"uppercase",marginBottom:6}}>Week Utilization</div><div className="stat-value" style={{fontSize:22,fontWeight:700,fontFamily:M,lineHeight:1,letterSpacing:-0.5,fontVariantNumeric:"tabular-nums",color:util>=80?B.green:util>=60?B.orange:B.red}}>{util}%</div></div>
          <div className="stat-card" style={{flex:"1 1 130px",minWidth:130,padding:"14px 16px",background:B.surface,borderRadius:8,border:"1px solid "+B.border,boxSizing:"border-box"}}><div className="stat-label" style={{fontSize:11,color:B.textMuted,fontWeight:600,letterSpacing:0.3,textTransform:"uppercase",marginBottom:6}}>Week Hours</div><div className="stat-value" style={{fontSize:22,fontWeight:700,fontFamily:M,lineHeight:1,letterSpacing:-0.5,fontVariantNumeric:"tabular-nums",color:B.text}}>{fmtHours(weekHrs)}</div></div>
        </div>;})()}
      {recentWOs.length>0&&<div style={{marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:6}}>{recentWOs.map(wo=>{const sc=wo.status==="completed"?B.green:wo.status==="in_progress"?B.cyan:B.orange;return <div key={wo.id} onClick={()=>{setTab("orders");setNavWOId(wo.id);haptic(15);}} style={{minWidth:148,padding:"12px 14px",background:B.surface,border:"1px solid "+B.border,borderRadius:8,cursor:"pointer",flexShrink:0,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",transition:"transform .15s, box-shadow .15s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 4px 12px rgba(0,0,0,0.12)";}} onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 1px 4px rgba(0,0,0,0.06)";}}>
      ...
      {myActive.length===0&&<EmptyState icon="check" title="All caught up!" subtitle="No active work orders right now. Enjoy the downtime."/>}
      ...
      <button onClick={()=>{if(!qlWO)setQlWO(myActive.length===1?myActive[0].id:(recentWOs.find(w=>w.status!=="completed")?.id||""));setQuickLog(true);haptic(30);}} data-tour="quick-log" title="Quick log time" style={{position:"fixed",bottom:"max(84px, calc(66px + env(safe-area-inset-bottom)))",right:14,width:52,height:52,borderRadius:"50%",background:B.cyan,border:"none",color:B.bg,cursor:"pointer",boxShadow:"0 4px 14px rgba(0,0,0,0.28)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:150,transition:"transform .15s"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.08)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}><Icon name="clock" size={24}/></button>
    ...
    {tab==="planner"&&<DayPlanner wos={D.wos} templates={D.templates} users={D.users} userName={user.name} userRole={user.role} onOpenWO={(id)=>{setTab("orders");setNavWOId(id);}} onUpdateWO={A.updateWO} canReschedule={user.role!=="technician"} schedule={D.schedule||[]} onAddSchedule={A.addSchedule} onDeleteSchedule={A.deleteSchedule} canAssignOthers={user.role!=="technician"}/>}
...
function MgrDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  ...
  return(<Shell user={user} onLogout={onLogout} tab={tab} setTab={setTab} syncing={syncing} offlineQueueCount={offlineQueueCount} notifications={visibleNotifs(D.notifs,user.role)} onMarkRead={A.markRead} onQuickApprovePO={A.quickApprovePO} onQuickRejectPO={A.quickRejectPO} onNavigateWO={(woId)=>{setTab("orders");if(woId)setNavWOId(woId);}} onRefresh={A.loadData} searchData={{wos:D.wos,pos:D.pos,customers:D.customers,equipment:D.equipment,projects:D.projects}} tabs={[{key:"overview",label:"Overview",icon:"chart"},{key:"inbox",label:"Requests"+(pendingDrafts?" ("+pendingDrafts+")":""),icon:"inbox"},{key:"orders",label:"Work Orders",icon:"clipboard"},{key:"planner",label:"Week Plan",icon:"calendar"},{key:"pos",label:"PO Mgmt",icon:"file"},{key:"rfqs",label:"RFQs",icon:"mail"},{key:"audit",label:"Supply Audit",icon:"receipt"},{key:"pricebook",label:"Price Book",icon:"tag"},{key:"reports",label:"Reports",icon:"chart"},{key:"billing",label:"Billing",icon:"dollar"},{key:"invoices",label:"Invoices",icon:"edit"},{key:"parts",label:"Parts Sales",icon:"box"},{key:"feedback",label:"Feedback",icon:"star"},{key:"agreements",label:"Agreements",icon:"clipboard"},{key:"equipment",label:"Equipment",icon:"wrench"},{key:"team",label:"Team",icon:"user"},{key:"customers",label:"Customers",icon:"building"},{key:"users",label:"Users",icon:"user"},{key:"calendar",label:"Calendar",icon:"calendar"},{key:"projects",label:"Projects",icon:"building"},{key:"kb",label:"Knowledge",icon:"book"},{key:"guide",label:"Guide",icon:"book"}]}>
    {tab==="overview"&&<><KPIDashboard D={D} A={A} userRole={user.role} userName={user.name} onOpenWO={(woId)=>{setTab("orders");setNavWOId(woId);}} onOpenInvoices={()=>setTab("invoices")}/>{pendingDrafts>0&&<Card onClick={()=>setTab("inbox")} style={{padding:"14px 18px",marginBottom:12,cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div><div style={{fontSize:14,fontWeight:700,color:B.text}}>Service Requests</div><div style={{fontSize:11,color:B.textMuted}}>{pendingDrafts} pending review</div></div></div><span style={{background:B.orange,color:B.bg,padding:"4px 10px",borderRadius:8,fontSize:13,fontWeight:700,fontFamily:M}}>{pendingDrafts}</span></div></Card>}<RepeatFailures wos={D.wos} pos={D.pos} equipment={D.equipment}/><WOOverview orders={D.wos} wlp={wlp} pos={D.pos} time={D.time}/><GlobalActivityFeed/></>}
    ...
    {tab==="pricebook"&&<PriceBook userRole={user.role}/>}
    ...
    {tab==="team"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>{D.users.filter(u=>u.role==="technician"&&u.active!==false).map(t=>{const to=D.wos.filter(o=>o.assignee===t.name);return(<Card key={t.id} style={{padding:"14px 18px"}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:42,height:42,borderRadius:8,background:B.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",color:B.cyan,fontSize:14,fontWeight:700}}>{t.name.split(" ").map(n=>n[0]).join("")}</div><div style={{flex:1}}><div style={{fontSize:15,fontWeight:700,color:B.text}}>{t.name}</div><div style={{fontSize:11,color:B.textDim}}>{to.filter(o=>o.status==="in_progress").length} active · {to.filter(o=>o.status==="completed").length} done · {fmtHours(to.reduce((s,o)=>s+calcWOHours(o.id,D.time),0))}</div></div>{(D.onlineUsers||[]).includes(t.name)?<Badge color={B.green}>Online</Badge>:<Badge color={B.textDim}>Offline</Badge>}</div></Card>);})}</div>}
    ...
    {tab==="projects"&&<Projects projects={D.projects||[]} users={D.users} customers={D.customers} userName={user.name} userRole={user.role} onAdd={A.addProject} onUpdate={A.updateProject} onDelete={A.deleteProject} allWOs={D.wos} onCreateWO={A.createWO} onUpdateWO={A.updateWO} onDeleteWO={A.deleteWO} allPOs={D.pos} onCreatePO={A.createPO} allTime={D.time} lineItems={D.lineItems||[]} photos={D.photos} onAddTime={A.addTime} onUpdateTime={A.updateTime} onDeleteTime={A.deleteTime} onAddPhoto={A.addPhoto} equipment={D.equipment||[]} invoices={D.invoices||[]} emailTemplates={D.emailTemplates||[]} onCreateInvoice={A.createInvoice} currentUser={user} loadData={A.loadData} reloadTable={A.reloadTable}/>}
    {tab==="calendar"&&<CompanyCalendar userRole={user.role} wos={D.wos} userName={user.name} time={D.time} schedule={D.schedule||[]} users={D.users||[]}/>}
...
function AdminDash({user,onLogout,D,A,syncing,offlineMode,offlineQueueCount}){
  ...
  const pendingDrafts=(D.woDrafts||[]).filter(d=>d.status==="pending_review").length;
  const wlp={canEdit:true,pos:D.pos,onCreatePO:A.createPO,onUpdateWO:A.updateWO,onDeleteWO:A.deleteWO,onCreateWO:A.createWO,timeEntries:D.time,photos:D.photos,onAddTime:A.addTime,onUpdateTime:A.updateTime,onDeleteTime:A.deleteTime,onAddPhoto:A.addPhoto,users:D.users,customers:D.customers,equipment:D.equipment||[],lineItems:D.lineItems||[],userName:user.name,userRole:user.role,loadData:A.loadData,reloadTable:A.reloadTable,navWOId,clearNavWO:()=>setNavWOId(null),invoices:D.invoices||[],projects:D.projects||[],emailTemplates:D.emailTemplates||[],onCreateInvoice:A.createInvoice,currentUser:user};
  return(<Shell user={user} onLogout={onLogout} tab={tab} setTab={setTab} syncing={syncing} offlineQueueCount={offlineQueueCount} notifications={visibleNotifs(D.notifs,user.role)} onMarkRead={A.markRead} onQuickApprovePO={A.quickApprovePO} onQuickRejectPO={A.quickRejectPO} onNavigateWO={(woId)=>{setTab("orders");if(woId)setNavWOId(woId);}} onRefresh={A.loadData} searchData={{wos:D.wos,pos:D.pos,customers:D.customers,equipment:D.equipment,projects:D.projects}} tabs={[{key:"overview",label:"Overview",icon:"chart"},{key:"inbox",label:"Requests"+(pendingDrafts?" ("+pendingDrafts+")":""),icon:"inbox"},{key:"orders",label:"All Orders",icon:"clipboard"},{key:"planner",label:"Week Plan",icon:"calendar"},{key:"pos",label:"PO Mgmt",icon:"file"},{key:"rfqs",label:"RFQs",icon:"mail"},{key:"audit",label:"Supply Audit",icon:"receipt"},{key:"pricebook",label:"Price Book",icon:"tag"},{key:"reports",label:"Reports",icon:"chart"},{key:"billing",label:"Billing",icon:"dollar"},{key:"invoices",label:"Invoices",icon:"edit"},{key:"parts",label:"Parts Sales",icon:"box"},{key:"feedback",label:"Feedback",icon:"star"},{key:"proposals",label:"Proposals",icon:"file"},{key:"agreements",label:"Agreements",icon:"clipboard"},{key:"recurring",label:"PM Schedule",icon:"repeat"},{key:"equipment",label:"Equipment",icon:"wrench"},{key:"customers",label:"Customers",icon:"building"},{key:"users",label:"Users",icon:"user"},{key:"team",label:"Team",icon:"user"},{key:"settings",label:"Settings",icon:"settings"},{key:"calendar",label:"Calendar",icon:"calendar"},{key:"projects",label:"Projects",icon:"building"},{key:"kb",label:"Knowledge",icon:"book"},{key:"guide",label:"Guide",icon:"book"}]}>
    {tab==="overview"&&<><KPIDashboard D={D} A={A} userRole={user.role} userName={user.name} onOpenWO={(woId)=>{setTab("orders");setNavWOId(woId);}} onOpenInvoices={()=>setTab("invoices")}/>{pendingDrafts>0&&<Card onClick={()=>setTab("inbox")} style={{padding:"14px 18px",marginBottom:12,cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div><div style={{fontSize:14,fontWeight:700,color:B.text}}>Service Requests</div><div style={{fontSize:11,color:B.textMuted}}>{pendingDrafts} pending review</div></div></div><span style={{background:B.orange,color:B.bg,padding:"4px 10px",borderRadius:8,fontSize:13,fontWeight:700,fontFamily:M}}>{pendingDrafts}</span></div></Card>}<RepeatFailures wos={D.wos} pos={D.pos} equipment={D.equipment}/><WOOverview orders={D.wos} wlp={wlp} pos={D.pos} time={D.time}/><GlobalActivityFeed/></>}
    {tab==="inbox"&&<ServiceRequests drafts={D.woDrafts||[]} customers={D.customers} users={D.users} onApprove={A.approveDraft} onReject={A.rejectDraft} onRefresh={A.loadData}/>}
    {tab==="orders"&&<WOList orders={D.wos} {...wlp}/>}
    {tab==="planner"&&<DayPlanner wos={D.wos} templates={D.templates} users={D.users} userName={user.name} userRole={user.role} onOpenWO={(id)=>{setTab("orders");setNavWOId(id);}} onUpdateWO={A.updateWO} canReschedule={user.role!=="technician"} schedule={D.schedule||[]} onAddSchedule={A.addSchedule} onDeleteSchedule={A.deleteSchedule} canAssignOthers={user.role!=="technician"}/>}
