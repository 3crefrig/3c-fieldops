...
import { Card, Badge, StatCard, Modal, Toast, Spinner, SkeletonLoader, EmptyState, CustomSelect, Icon } from "./ui";
...
function BillingExport({wos,pos,timeEntries,customers,emailTemplates,currentUser}){
  ...
  const buildTimesheetXLSX=async()=>{
    ...
    ws.columns=[{width:14},{width:14},{width:12},{width:16},{width:14},{width:40}];
    const thinBorder={style:"thin",color:{argb:"FFDDDDDD"}};
    const headerFill={type:"pattern",pattern:"solid",fgColor:{argb:"FF00B7E8"}};
    const totalFill={type:"pattern",pattern:"solid",fgColor:{argb:"FFE8F5E9"}};
    // Header row
    const hdr=ws.addRow(["Date:","Building #","Room#","WO/Asset#","Personnel Hrs.","Description"]);
    hdr.height=22;hdr.eachCell(c=>{c.font={name:"Calibri",size:13,bold:true,color:{argb:"FFFFFFFF"}};c.fill=headerFill;c.alignment={horizontal:"center",vertical:"middle"};c.border={top:{style:"medium"},bottom:{style:"medium"}};});
    // Data rows
    rows.forEach(r=>{const row=ws.addRow([r.date,r.building,r.room,r.wo_num,r.hours,r.desc||""]);
      row.eachCell((c,i)=>{c.font={name:"Calibri",size:11,bold:i===5};c.border={bottom:thinBorder};});
      row.getCell(5).numFmt="0.00";});
    // Total row
    const totRow=ws.addRow(["","","","TOTAL:",totalHrs,""]);
    totRow.getCell(4).font={name:"Calibri",size:12,bold:true};totRow.getCell(4).fill=totalFill;
    totRow.getCell(5).font={name:"Calibri",size:12,bold:true};totRow.getCell(5).fill=totalFill;totRow.getCell(5).numFmt="0.00";
    return wb;
  };
  const buildXLSXBase64=async()=>{const wb=await buildTimesheetXLSX();const buf=await wb.xlsx.writeBuffer();return btoa(String.fromCharCode(...new Uint8Array(buf)));};
  const generateXLSX=async()=>{
    try{const wb=await buildTimesheetXLSX();const buf=await wb.xlsx.writeBuffer();
      const blob=new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"});
      const url=URL.createObjectURL(blob);const a=document.createElement("a");
      a.href=url;a.download="3C_Timesheet"+(custFilter?"_"+custFilter.replace(/\s/g,"_"):"")+(dateFrom&&dateTo?"_"+dateFrom+"_to_"+dateTo:dateFrom?"_"+dateFrom:"")+".xlsx";
      a.click();URL.revokeObjectURL(url);setToast("Downloaded");setTimeout(()=>setToast(""),3000);
    }catch(e){setToast("Error: "+e.message);console.error(e);}
  };
  ...
  const sendTimesheet=async()=>{if(!emailTo.trim()||sending)return;setSending(true);const fname="3C_Timesheet"+(custFilter?"_"+custFilter.replace(/\s/g,"_"):"")+(dateFrom&&dateTo?"_"+dateFrom+"_to_"+dateTo:dateFrom?"_"+dateFrom:"")+".xlsx";const xlsB64=await buildXLSXBase64();const{rows,totalHrs}=getTimesheetRows();let tbl='<table style="border-collapse:collapse;width:100%;margin:16px 0;"><tr style="background:#00B7E8;color:#fff;">';["Date","Building #","Room#","WO/Asset#","Hrs.","Description"].forEach(c=>{tbl+='<th style="padding:8px 12px;text-align:left;border:1px solid #ddd;">'+c+'</th>';});tbl+='</tr>';rows.forEach((r,i)=>{tbl+='<tr style="background:'+(i%2===0?'#f9f9f9':'#fff')+';"><td style="padding:6px 12px;border:1px solid #ddd;">'+r.date+'</td><td style="padding:6px 12px;border:1px solid #ddd;">'+r.building+'</td><td style="padding:6px 12px;border:1px solid #ddd;">'+r.room+'</td><td style="padding:6px 12px;border:1px solid #ddd;">'+r.wo_num+'</td><td style="padding:6px 12px;border:1px solid #ddd;font-weight:bold;">'+r.hours+'</td><td style="padding:6px 12px;border:1px solid #ddd;">'+r.desc+'</td></tr>';});tbl+='<tr style="background:#E8F5E9;font-weight:bold;"><td colspan="4" style="padding:8px 12px;border:1px solid #ddd;text-align:right;">TOTAL:</td><td style="padding:8px 12px;border:1px solid #ddd;">'+parseFloat(totalHrs.toFixed(2))+'</td><td></td></tr></table>';const fullBody='<div style="font-family:Calibri,sans-serif;">'+emailBody+tbl+buildSig()+'</div>';try{const emails=emailTo.split(",").map(e=>e.trim()).filter(Boolean);for(const em of emails){await saveContact(em);}if(emailCC){emailCC.split(",").map(e=>e.trim()).filter(Boolean).forEach(em=>saveContact(em));}const resp=await fnFetch("send-email",{to:emailTo.trim(),cc:emailCC.trim(),subject:emailSubject+(custFilter?" for "+custFilter:"")+(dateFrom&&dateTo?" ("+dateFrom+" to "+dateTo+")":dateFrom?" ("+dateFrom+")":""),body:fullBody,attachment:{name:fname,content:xlsB64,type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}});const result=await resp.json();if(result.success){setToast("Email sent!");setShowEmail(false);setEmailTo("");setEmailCC("");sb().from("email_contacts").select("*").order("last_used",{ascending:false}).then(({data})=>{if(data)setContacts(data);});}else{setToast("Error: "+(result.error||"Failed"));console.error(result);}}catch(err){setToast("Error sending");console.error(err);}setSending(false);setTimeout(()=>setToast(""),4000);};
  ...
  const _shift=(days)=>_ymd(new Date(_today.getFullYear(),_today.getMonth(),_today.getDate()+days));
  const _monthStart=(off)=>_ymd(new Date(_today.getFullYear(),_today.getMonth()+off,1));
  const _monthEnd=(off)=>_ymd(new Date(_today.getFullYear(),_today.getMonth()+off+1,0));
  const _qStart=()=>_ymd(new Date(_today.getFullYear(),Math.floor(_today.getMonth()/3)*3,1));
  const datePresets=[["all","All Time","",""],["today","Today",_shift(0),_shift(0)],["yesterday","Yesterday",_shift(-1),_shift(-1)],["7d","Last 7 Days",_shift(-6),_shift(0)],["month","This Month",_monthStart(0),_shift(0)],["lastMonth","Last Month",_monthStart(-1),_monthEnd(-1)],["quarter","This Quarter",_qStart(),_shift(0)],["year","This Year",_today.getFullYear()+"-01-01",_shift(0)]];
  const activeDatePreset=datePresets.find(p=>p[2]===dateFrom&&p[3]===dateTo)?.[0];
  return(<div><Toast msg={toast}/>
    <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:600,color:B.text}}>Customer Billing Export</h3>
    <div style={{marginBottom:10}}>
      <span style={LS}>Date Range</span>
      <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>{datePresets.map(([k,label,f,t])=><button key={k} onClick={()=>{setDateFrom(f);setDateTo(t);}} style={{padding:"6px 12px",borderRadius:4,border:"1px solid "+(activeDatePreset===k?B.cyan:B.border),background:activeDatePreset===k?B.cyanGlow:"transparent",color:activeDatePreset===k?B.cyan:B.textDim,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:F}}>{label}</button>)}</div>
    </div>
    <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
      <div><label style={LS}>From</label><input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={{...IS,borderColor:dateFrom?B.cyan:B.border,boxShadow:dateFrom?"0 0 0 1px "+B.cyan+"33":"none"}}/></div>
      <div><label style={LS}>To</label><input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={{...IS,borderColor:dateTo?B.cyan:B.border,boxShadow:dateTo?"0 0 0 1px "+B.cyan+"33":"none"}}/></div>
    ...
    <div style={{marginBottom:10}}>
      ...
      <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
        {[["basic","Basic","WO#, Date, Customer, Hours"],["detailed","Detailed","+ Title, Location, Building, POs"],["full","Full","All columns"]].map(([k,label,desc])=><button key={k} onClick={()=>setCols(presets[k])} style={{flex:"1 1 150px",padding:"10px 8px",borderRadius:6,border:"2px solid "+(activePreset===k?B.cyan:B.border),background:activePreset===k?B.cyanGlow:"transparent",cursor:"pointer",textAlign:"center"}}><div style={{fontSize:12,fontWeight:700,color:activePreset===k?B.cyan:B.text}}>{label}</div><div style={{fontSize:10.5,color:B.textDim,marginTop:2}}>{desc}</div></button>)}
      </div>
    ...
    {/* Billing Stats */}
    {(()=>{const totalHrs=completed.reduce((s,wo)=>{const ft=timeEntries.filter(t=>t.wo_id===wo.id).filter(t=>{const d=t.logged_date||wo.date_completed;return(!dateFrom||d>=dateFrom)&&(!dateTo||d<=dateTo);});return s+ft.reduce((ss,t)=>ss+parseFloat(t.hours||0),0);},0);const totalPOs=completed.reduce((s,wo)=>s+pos.filter(p=>p.wo_id===wo.id&&p.status==="approved").reduce((ss,p)=>ss+parseFloat(p.amount||0),0),0);const pendingPOs=completed.reduce((s,wo)=>s+pos.filter(p=>p.wo_id===wo.id&&p.status==="pending").length,0);const avgHrs=completed.length>0?totalHrs/completed.length:0;return(<div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}><StatCard label="Work Orders" value={completed.length} icon="clipboard" color={B.cyan}/><StatCard label="Total Hours" value={fmtHours(totalHrs)} icon="clock" color={B.orange}/><StatCard label="Avg Hrs/WO" value={fmtHours(avgHrs)} icon="chart" color={B.green}/><StatCard label="PO Total" value={"$"+totalPOs.toFixed(2)} icon="dollar" color={B.cyan}/>{pendingPOs>0&&<StatCard label="Pending POs" value={pendingPOs} icon="clock" color={B.red}/>}</div>);})()}
    <Card style={{overflowX:"auto",marginBottom:14,padding:0}}><table className="data-table" style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{borderBottom:"2px solid "+B.border}}>{cols.map(c=><th key={c} style={{textAlign:"left",padding:"8px 10px",color:B.textMuted,fontWeight:600,fontSize:10.5,letterSpacing:0.4,textTransform:"uppercase",whiteSpace:"nowrap"}}>{allCols[c]}</th>)}</tr></thead><tbody>{completed.map(wo=>{const d=getData(wo);return(<tr key={wo.id} onClick={()=>openWO(wo.wo_id||wo.id)} title="Open this work order" style={{borderBottom:"1px solid "+B.border,cursor:"pointer"}}>{cols.map(c=><td key={c} style={{padding:"8px 10px",fontFamily:c==="wo_id"||c==="customer_wo"||c==="hours"||c==="po_total"||c==="date_completed"?M:F,color:c==="wo_id"?B.cyan:c==="hours"?B.cyan:B.text,whiteSpace:["wo_id","customer_wo","date_completed","hours","po_total","wo_type","building"].includes(c)?"nowrap":"normal",fontSize:c==="title"||c==="customer"?12:11.5}}>{c==="hours"?d.hours_display:d[c]}</td>)}</tr>);})}</tbody></table></Card>
    <div style={{display:"flex",gap:8}}>
      <button data-tip="Copies the table ready to paste into a spreadsheet or email — columns keep their layout." onClick={copyToClip} style={{...BP,flex:1}}>Copy</button>
      <button onClick={generateXLSX} style={{...BP,flex:1,background:B.green}}>Download</button>
      <button onClick={()=>setShowEmail(true)} style={{...BP,flex:1,background:B.cyan}}>Email</button>
    </div>
    {showEmail&&<Modal title="Send Timesheet via Email" onClose={()=>setShowEmail(false)} wide>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{background:B.bg,borderRadius:6,padding:10,border:"1px solid "+B.border,fontSize:12,color:B.textMuted}}>Attaching 3C Timesheet (.xlsx) with {completed.length} entries{custFilter?" for "+custFilter:""} from <span style={{color:B.cyan}}>service@3crefrigeration.com</span></div>
        {(()=>{const warnings=[];const zeroHrs=completed.filter(wo=>{const ft=timeEntries.filter(t=>t.wo_id===wo.id).filter(t=>{const d=t.logged_date||wo.date_completed;return(!dateFrom||d>=dateFrom)&&(!dateTo||d<=dateTo);});return ft.reduce((s,t)=>s+parseFloat(t.hours||0),0)===0;});const noDesc=completed.filter(wo=>!wo.work_performed&&!wo.notes||wo.notes==="No details.");const noDate=completed.filter(wo=>!wo.date_completed);if(zeroHrs.length>0)warnings.push({icon:"clock",msg:zeroHrs.length+" WO"+(zeroHrs.length>1?"s have":" has")+" 0 hours logged: "+zeroHrs.map(w=>w.wo_id).join(", ")});if(noDesc.length>0)warnings.push({icon:"edit",msg:noDesc.length+" WO"+(noDesc.length>1?"s have":" has")+" no work description: "+noDesc.map(w=>w.wo_id).join(", ")});if(noDate.length>0)warnings.push({icon:"calendar",msg:noDate.length+" WO"+(noDate.length>1?"s have":" has")+" no completion date"});return warnings.length>0?<div style={{display:"flex",flexDirection:"column",gap:4}}>{warnings.map((w,i)=><div key={i} style={{background:B.orange+"15",border:"1px solid "+B.orange+"33",borderRadius:6,padding:"8px 12px",fontSize:11,color:B.orange,display:"flex",alignItems:"center",gap:8}}><span style={{display:"inline-flex"}}><Icon name={w.icon||"alert"} size={13}/></span><span>{w.msg}</span></div>)}</div>:null;})()}
        ...
        <div style={{display:"flex",gap:8}}><button onClick={()=>setShowEmail(false)} style={{...BS,flex:1}}>Cancel</button><button onClick={sendTimesheet} disabled={sending} style={{...BP,flex:1,opacity:sending?.6:1}}>{sending?"Sending...":"Send"}</button></div>
      </div>
    </Modal>}
  </div>);
}
