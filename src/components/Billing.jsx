...
function BillingExport({wos,pos,timeEntries,customers,emailTemplates,currentUser}){
  ...
  return(<div><Toast msg={toast}/>
    <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:700,color:B.text}}>Customer Billing Export</h3>
    ...
    <Card style={{overflowX:"auto",marginBottom:14}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{borderBottom:"2px solid "+B.border}}>{cols.map(c=><th key={c} style={{textAlign:"left",padding:"8px 10px",color:B.textMuted,fontWeight:600,fontSize:10.5,letterSpacing:0.4,textTransform:"uppercase",whiteSpace:"nowrap"}}>{allCols[c]}</th>)}</tr></thead><tbody>{completed.map(wo=>{const d=getData(wo);return(<tr key={wo.id} onClick={()=>openWO(wo.wo_id||wo.id)} title="Open this work order" style={{borderBottom:"1px solid "+B.border,cursor:"pointer"}}>{cols.map(c=><td key={c} style={{padding:"8px 10px",fontFamily:c==="wo_id"||c==="customer_wo"||c==="hours"||c==="po_total"||c==="date_completed"?M:F,color:c==="wo_id"?B.cyan:c==="hours"?B.cyan:B.text,whiteSpace:["wo_id","customer_wo","date_completed","hours","po_total","wo_type","building"].includes(c)?"nowrap":"normal",fontSize:c==="title"||c==="customer"?12:11.5}}>{c==="hours"?d.hours_display:d[c]}</td>)}</tr>);})}</tbody></table></Card>
    ...
    {showEmail&&<Modal title="Send Timesheet via Email" onClose={()=>setShowEmail(false)} wide>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{display:"flex",gap:8}}><button onClick={()=>setShowEmail(false)} style={{...BS,flex:1}}>Cancel</button><button onClick={sendTimesheet} disabled={sending} style={{...BP,flex:1,background:B.cyan,opacity:sending?.6:1}}>{sending?"Sending...":"Send"}</button></div>
      </div>
    </Modal>}
  </div>);
}
