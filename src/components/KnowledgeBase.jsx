...
function KnowledgeBase({ userName, userRole }) {
  ...
  if (selArticle && !showCreate) {
    ...
    return (<div><Toast msg={toast} />
      ...
      {a.symptoms && <Card style={{ padding: 14, marginBottom: 12 }}><span style={{ fontSize: 10, fontWeight: 700, color: a.category === "sop" ? B.red : B.orange, textTransform: "uppercase" }}>{a.category === "sop" ? "Scope / Purpose" : "Symptoms"}</span><RichText text={a.symptoms} photos={aPhotos} /></Card>}
      {a.fix_steps && <Card style={{ padding: 14, marginBottom: 12 }}><span style={{ fontSize: 10, fontWeight: 700, color: a.category === "sop" ? B.red : B.green, textTransform: "uppercase" }}>{a.category === "sop" ? "Procedure" : "Fix / Steps"}</span><RichText text={a.fix_steps} photos={aPhotos} /></Card>}
      {a.content && <Card style={{ padding: 14, marginBottom: 12 }}><RichText text={a.content} photos={aPhotos} /></Card>}
      {(a.part_number || a.supplier) && <Card style={{ padding: 14, marginBottom: 12 }}><span style={{ fontSize: 10, fontWeight: 700, color: B.cyan, textTransform: "uppercase" }}>Parts Info</span>{a.part_number && <div style={{ marginTop: 6, fontSize: 13, color: B.text }}>Part #: <strong style={{ fontFamily: M }}>{a.part_number}</strong></div>}{a.supplier && <div style={{ fontSize: 13, color: B.text }}>Supplier: <strong>{a.supplier}</strong></div>}</Card>}
      {aPhotos.length > 0 && <Card style={{ padding: 14, marginBottom: 12 }}><span style={{ fontSize: 10, fontWeight: 700, color: B.textDim, textTransform: "uppercase" }}>Photos</span><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: 8, marginTop: 8 }}>{aPhotos.map((f, i) => <div key={f.id} style={{ borderRadius: 8, overflow: "hidden", border: "1px solid " + B.border, position: "relative" }}><span style={{ position: "absolute", top: 4, left: 4, background: "rgba(0,0,0,0.65)", color: "#fff", fontSize: 10, fontWeight: 700, padding: "1px 6px", borderRadius: 4, fontFamily: M }}>#{i + 1}</span><a href={f.file_url} target="_blank" rel="noreferrer"><img src={f.thumbnail_url || f.file_url} alt="" style={{ width: "100%", height: 100, objectFit: "cover" }} /></a><div style={{ padding: "4px 6px", background: B.surface }}>{f.caption && <div style={{ fontSize: 10, color: B.textMuted, marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.caption}</div>}<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 10, color: B.textDim }}>{f.uploaded_by}</span><span>{isMgr && <button onClick={() => editCaption(f, a.id)} style={{ background: "none", border: "none", color: B.cyan + "AA", fontSize: 10, cursor: "pointer" }}><Icon name="edit" size={12}/></button>}{isMgr && <button onClick={() => deleteFile(f.id, a.id)} style={{ background: "none", border: "none", color: B.red + "66", fontSize: 12, cursor: "pointer" }}>×</button>}</span></div></div></div>)}</div><div style={{ fontSize: 10, color: B.textDim, marginTop: 8 }}>Tip: edit the article and type <span style={{ fontFamily: M, color: B.cyan }}>[photo:2]</span> anywhere in the text to show photo #2 at that exact spot.</div></Card>}
      {aDocs.length > 0 && <Card style={{ padding: 14, marginBottom: 12 }}><span style={{ fontSize: 10, fontWeight: 700, color: B.textDim, textTransform: "uppercase" }}>Files & Documents</span>{aDocs.map(f => <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid " + B.border }}><span style={{ fontSize: 16 }}><Icon name="file" size={14}/></span><a href={f.file_url} target="_blank" rel="noreferrer" style={{ flex: 1, fontSize: 12, fontWeight: 600, color: B.cyan, textDecoration: "none" }}>{f.file_name}</a><span style={{ fontSize: 10, color: B.textDim }}>{f.uploaded_by}</span>{isMgr && <button onClick={() => deleteFile(f.id, a.id)} style={{ background: "none", border: "none", color: B.red + "66", fontSize: 12, cursor: "pointer" }}>×</button>}</div>)}</Card>}
      {a.file_url && <Card style={{ padding: 14, marginBottom: 12 }}><a href={a.file_url} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, color: B.cyan, textDecoration: "none" }}><span style={{ fontSize: 20 }}><Icon name="file" size={18}/></span><div><div style={{ fontSize: 13, fontWeight: 600 }}>{a.file_name || "Attached File"}</div><div style={{ fontSize: 10, color: B.textDim }}>Tap to view/download</div></div></a></Card>}
  ...
  return (<div><Toast msg={toast} />
    ...
    {!loading && filtered.length === 0 && <Card style={{ textAlign: "center", padding: 30, color: B.textDim }}><div style={{ fontSize: 24, marginBottom: 6 }}><Icon name="book" size={22}/></div><div style={{ fontSize: 13 }}>{search ? "No results for \"" + search + "\"" : "No articles yet. Add one to get started."}</div></Card>}
    {filtered.map(a => <Card key={a.id} onClick={() => setSelArticle(a)} style={{ padding: "14px 16px", marginBottom: 8, cursor: "pointer" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><Badge color={catColors[a.category]}>{cats[a.category]?.split(" ")[1] || a.category}</Badge>{a.status === "pending" && <Badge color={B.orange}>Pending</Badge>}{a.tags && a.tags.slice(0, 3).map(t => <span key={t} style={{ fontSize: 10, color: B.textDim, background: B.bg, padding: "1px 6px", borderRadius: 3 }}>{t}</span>)}</div>
        ...
        {a.file_url && <span style={{ fontSize: 16, flexShrink: 0 }}><Icon name="paperclip" size={14}/></span>}
      </div>
    </Card>)}
    {showCreate && <Modal title={selArticle ? "Edit Article" : "New Knowledge Base Article"} onClose={() => { setShowCreate(false); setSelArticle(null); }} wide>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div><label style={LS}>Photos <span style={{ color: B.textDim, fontWeight: 400 }}>(type [photo:1] in any text box to place photo #1 inline)</span></label>
          {editorPhotos.length > 0 && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(90px,1fr))", gap: 6, marginBottom: 8 }}>{editorPhotos.map((f, i) => <div key={f.id || i} style={{ borderRadius: 6, overflow: "hidden", border: "1px solid " + B.border, position: "relative" }}><span style={{ position: "absolute", top: 3, left: 3, background: "rgba(0,0,0,0.65)", color: "#fff", fontSize: 10, fontWeight: 700, padding: "1px 5px", borderRadius: 3, fontFamily: M }}>#{i + 1}</span><img src={f.thumbnail_url || f.file_url} alt="" style={{ width: "100%", height: 64, objectFit: "cover", display: "block" }} /><button onClick={() => selArticle ? deleteFile(f.id, selArticle.id) : removePending(pendingFiles.indexOf(f))} style={{ position: "absolute", top: 3, right: 3, background: "rgba(0,0,0,0.65)", border: "none", color: "#fff", fontSize: 10, cursor: "pointer", borderRadius: 3, padding: "1px 5px" }}>×</button></div>)}</div>}
