...
import { sb, SUPABASE_URL, SUPABASE_ANON_KEY, B, F, M, IS, LS, BP, BS, cleanText , fnFetch } from "../shared";
...
function KnowledgeBase({userName,userRole}){
  ...
  const save=async()=>{if(!title.trim()||saving)return;if(cleanText(title,"Title")===null||cleanText(content,"Content")===null)return;setSaving(true);try{const obj={title:title.trim(),category,content:content.trim(),symptoms:symptoms.trim(),fix_steps:fixSteps.trim(),part_number:partNum.trim(),supplier:supplier.trim(),tags:tags.split(",").map(t=>t.trim()).filter(Boolean),status:isMgr?"approved":"pending",author:userName};if(selArticle){await sb().from("kb_articles").update({...obj,updated_at:new Date().toISOString()}).eq("id",selArticle.id);}else{await sb().from("kb_articles").insert(obj);}setSaving(false);setShowCreate(false);resetForm();setSelArticle(null);await load();msg(selArticle?"Updated":isMgr?"Published":"Submitted for approval");}catch(e){console.error(e);setSaving(false);}};
