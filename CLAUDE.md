...
3. **"Copeland K-Body Compressor — Valve Plate & Field Service"** (2026-07-24) — 5 custom diagrams + 1 real Copeland figure. Storage: `kb-files/kb/kbody-sop/`. K-body = Copelametic small 2-cyl **reed-valve** (~0.5–4 hp), 3C's everyday compressor. Covers reed/valve-plate service, the **two-center-bolt head** (AE4-1219: perimeter 300 in-lb / center 225 in-lb — K-only), and the **centrifuge-vs-oil-pump lube fork** (R22/mineral = no pump; HFC/POE = internal pump, net oil 15–61 psid). Real figure #2 is Copeland's K/L-Series reed cross-section (DWM D6.3.4 App Guidelines Fig 1, cropped via pdf-to-img+sharp — internal reference). Custom figs cross-checked vs AE4-1219 R18, AE4-1294, D6.3.4. **Do not confuse** US Copeland "K-body" (Copelametic frame letter, reed, small 2-cyl) with the European DWM designation where K is a *voltage* code.
...
- **Two-pass page fitting.** `buildWOPdfFitted()` (what the UI calls) builds at the `normal` density, and if that spills past one page rebuilds at `compact` — every vertical measurement lives in the `DENSITY` table, so compact shaves ~20% off the rhythm without touching the type hierarchy. The compact copy is kept **only if it actually saves a page**, so long jobs still get the room they need. Tune spacing by editing `DENSITY`, not by sprinkling literals back into the layout.
